interface IAjaxResultRole extends IResultBase {
    aspnetid: string;
};
...
agApp.controller('ctrl', ['$scope', '$http', 'toaster', 'gridpage',
    function ($scope: IMakeScopeBase, $http: ng.IHttpService, toaster, gridpage) {

        var id_name: string = "Id";
        var sn_name: string = "Name";
    ...
    $scope.Master_Submit = function () {
            ...
            if ($scope.edit_type == IEditType.insert) {
                ...
                $http.post(aj_MInsert, $scope.fd)
                    .success(function (data: IAjaxResultRole, status, headers, config) {
                        console.log(data);
                        if (data.result) {
