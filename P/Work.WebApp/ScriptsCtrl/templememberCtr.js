...
angular
    .module('angularApp')
    .controller('ctrl', ['$scope', '$http', 'gridpage', 'workService', '$state', function ($scope, $http, gridpage, workService, $state) {
        $scope.sd = {};
        ...
        $scope.Master_Grid_Delete = function () {
            ...
            if (ids.length > 0) {
                if (confirm(msg_Info_Is_SureDelete)) {
                    $http['delete'](apiConnection + '?ids=' + ids)
...
angular.module('angularApp')
    .controller('ctrl_edit', ['$scope', '$http', 'workService', '$sce', '$state', '$q', function ($scope, $http, workService, $sce, $state, $q) {
