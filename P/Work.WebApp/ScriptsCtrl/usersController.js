...
agApp.controller('ctrl', ['$scope', '$http', 'toaster', 'gridpage', function ($scope, $http, toaster, gridpage) {
    var id_name = "Id";
    var sn_name = "Id";
    $scope.fd = {};
    $scope.fd[id_name] = 0;
    ...
    $scope.Master_Grid_Delete = function () {
        ...
        if (sns.length > 0) {
            if (confirm(msg_Info_Is_SureDelete)) {
                $http.post(aj_MDelete, sns).success(function (data, status, headers, config) {
                    if (data.result) {
                        $scope.Init_Query();
                    }
                    else {
    ...
    $scope.Master_Submit = function () {
        if ($scope.edit_type == 1) {
            ...
            $http.post(gb_allpath + 'ajax_MasterInsert', $scope.fd).success(function (data, status, headers, config) {
                console.log(data);
                if (data.result) {
                    $scope.edit_type = 2;
        ...
        if ($scope.edit_type == 2) {
            ...
            $http.post(aj_MUpdata, $scope.fd).success(function (data, status, headers, config) {
                if (data.result) {
    ...
    $scope.Master_Open_Modify = function ($index) {
        ...
        $http.post(aj_MGet, { sn: get_sn }).success(function (data, status, headers, config) {
            if (data.result) {
                $scope.fd = data.data;
