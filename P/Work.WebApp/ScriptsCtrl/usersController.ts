interface IMakeScopeUsers extends IMakeScopeBase {
    isPassword: boolean;
};

interface IAjaxResultUsers extends IResultBase {
    aspnetid: string;
};
...
agApp.controller('ctrl', ['$scope', '$http', 'toaster', 'gridpage',
    function ($scope: IMakeScopeUsers, $http: ng.IHttpService, toaster, gridpage) {

        var id_name: string = "Id";
        var sn_name: string = "Id";
    ...
    $scope.Master_Submit = function () {
            if ($scope.edit_type == 1) {
                ...
                $http.post(gb_allpath + 'ajax_MasterInsert', $scope.fd)
                    .success(function (data: IAjaxResultUsers, status, headers, config) {
                        console.log(data);
                        if (data.result) {
                            $scope.edit_type = 2;
