angular.module('angularApp', ['commfun', 'siyfion.sfTypeahead']).controller('ctrl', ['$scope', '$http', 'workService', 'gridpage', function ($scope, $http, workService, gridpage) {
    ...
    $scope.openFamilyData = function (member_id) {
        workService.getMemberDetailAll(member_id).success(function (data, status, headers, config) {
            if (data.result) {
                $scope.isShow = true;
    ...
    $scope.upCheck = function (index) {
        ...
        $http.put(gb_approot + 'api/PutAction/PutTempleMemberCheck', { member_id: m.temple_member_id, repeat_mark: m.is_close }).success(function (data, status, headers, config) {
            if (!data.result) {
                alert(data.message);
            }
        }).error(function (data, status, headers, config) {
