interface IProduct extends IScopeM<server.Product> {
    sd: any;
    masterDelete($index): void //主檔單一筆刪除
}
interface IProductEdit extends IScopeM<server.Product> {
    closeEdit(): void;
    closeAcct(): void;
    submitData(): void;
    submitAcct(): void;
...
angular
    ...
    function (
        ...
        $scope.sd = {};
        $scope.apiPath = gb_approot + "api/Product";
        ...
        $scope.Master_Grid_Delete = function () {
            ...
            for (var key in $scope.Grid_Items) {
                if ($scope.Grid_Items[key].check_del) {
                    ids.push($scope.Grid_Items[key].product_sn);
                }
            }
        ...
        $scope.Master_Edit_Close = function () {
            ...
            $scope.show_master_edit = false;
            $scope.fd = <server.Product>{};
        };
        $scope.Master_Open_Modify = function ($index) {
            var get_id = $scope.Grid_Items[$index].product_sn;
            $state.go('edit', { 'product_sn': get_id });
        };
...
angular.module('angularApp')
    ...
    function (
            ...
            if (submitCheck) {
                if ($scope.edit_type == IEditType.insert) {
                    $http.post(gb_approot + 'api/TempleMember', $scope.fd)
                        .success(function (data: { result: boolean; message: string; data: any; id: string; }, status, headers, config) {
                        if (data.result) {
                            $scope.fd.product_sn = data.id;
                            getDetailListData($scope.fd.product_sn);
                            $scope.edit_type = IEditType.update;
                            alert('增新完成');
                        } else {
                            alert(data.message);
        ...
        $scope.submitAcct = function () {
            if ($scope.detail_edit_type == IEditType.insert) {
                ...
                $http.post(gb_approot + 'api/TempleAccount', $scope.fds)
                    .success(function (data: IResultBase, status, headers, config) {
                    if (data.result) {
                        getMasterData($scope.fds.product_sn);
                        getDetailListData($scope.fds.product_sn);
        ...
        function getDetailListData(id: string) {
        ...
        if ($state.params.temple_member_id != undefined) { // 進入為修改模式
            var get_id: string = $state.params.product_sn;
            $scope.edit_type = IEditType.update;
