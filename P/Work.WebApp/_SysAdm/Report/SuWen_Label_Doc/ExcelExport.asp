...
Dim strkey
	...
	'StrSql=StrSql & " A.數量, "
	'StrSql=StrSql & " A.價格, "
	StrSql=StrSql & " DISTINCT A.申請人姓名"
	...
	'======================================================
	'-- 福燈需要join table---------------------------------(2015/2/12)增加福燈列印功能 by Ajoe
	If intDocType=4 then
	StrSql=""
	StrSql=" Select "
	StrSql=StrSql & " DISTINCT B.member_name AS '申請人姓名' "
	StrSql=StrSql & " FROM 訂單明細檔 AS A"
	StrSql=StrSql & " inner join Fortune_Light AS B "
	StrSql=StrSql & " on A.訂單編號=B.order_sn "
	StrSql=StrSql & StrWhere
	end If
	'======================================================
	...
	intLabelColumns=10			'每列標籤數
	intLabelRows=12				'每頁標籤列數
	
	intLabelDataRows=1			'每個標籤資料所佔的ROW數 (橫向標籤為ROW)
	intLabelJumpRows=0			'每個標籤資料跳欄數(此值通常為 intLabelDataRows-1)
	...
	For intLoopPageNumber = 1 To intLoopPageCount
		...
		Dim intExcel_StartRows,intExcel_StartCols
		intExcel_StartRows=3
		intExcel_StartCols=1
		Call objXLS.PutArrayDataToEXCEL_RC(aryFieldsData,intExcel_StartRows,intExcel_StartCols,intPageNumber,1)						
